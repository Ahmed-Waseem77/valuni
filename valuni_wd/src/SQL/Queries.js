...
app.get("/getReviewContent/:crn", function (req, res) {
  ...
  console.log(sql); // Log the SQL query after replacing the parameters
  console.log(crn);
...
app.get("/getCourseInstructors/:crn", function (req, res) {
  ...
  const sql =
    "SELECT I.First_Name, I.Last_Name, I.ID " +
...
app.post('/insertReview', (req, res) => {
  const { User_Major, RText, User_Email, Semester, CRN, Grading, Workload, Difficulty, Support, ContentQuality, Inst_ID} = req.body;

  const sql = 'INSERT INTO Review (User_Major, RText, User_Email, Semester, CRN, Grading, Workload, Difficulty, Support, ContentQuality, Inst_ID) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)';

  con.query(sql, [User_Major, RText, User_Email, Semester, CRN, Grading, Workload, Difficulty, Support, ContentQuality, Inst_ID], (error, results) => {
    if (error) {
